...
import { sinSerie, getColorIndex, splitArr } from '../utils/common';


/**
 * AE相位图谱类
 */
class Phase extends Chart {
    constructor(...args) {
        super(...args);
    }
    /**
    ...
    option() {
        ...
        let { color } = this.chartBody.series[0];
        let visualMap = {};
        //设置图谱颜色
        if (isEmpty(color)) {
    ...
    getSeries(opts, chartBody) {
        ...
        let { xRangeMax, xRangeMin, yRangeMax, yRangeMin, zDesc } = axisInfo;

        let xRange = parseFloat(xRangeMax) - parseFloat(xRangeMin);
        let yRange = parseFloat(yRangeMax) - parseFloat(yRangeMin);
        let sinLine = sinSerie(xRange, xRangeMin, yRange, yRangeMin, opts.lineWidth);
        let seriesOption = [{
            ...sinLine,
            markLine: {
                silent: true,
